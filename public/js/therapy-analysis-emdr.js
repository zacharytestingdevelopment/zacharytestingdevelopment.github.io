...
function analyzeSession(setAnalyzed) {
    ...
    desc.on('value', function (snapshot) {
        ...
        if (snapshot.val().averageVoCAverage != "empty") {
            averageVoCResult = snapshot.val().averageVoCAverage;
            VoCResultsData = snapshot.val().setVoCProgress;
            calculateSessionCount(snapshot.val().setVoCProgress, "VoC");
            convertToInt(VoCResultsArray);

            VoCChartActive = true;
        }

        longestData(convertToInt(moodResultsArray), convertToInt(recallResultsArray), convertToInt(sudsResultsArray), convertToInt(VoCResultsArray));
        ...
        if ((averageSudsResult != "empty" && sudsResultsData.length > 0) && (averageVoCResult != "empty" && VoCResultsData.length > 0)) {
            ...
            document.getElementById("average-VoC").innerHTML = calculateAverageScore(VoCResultsData);
        }
        else if ((averageSudsResult != "empty" && sudsResultsData.length > 0) && (averageVoCResult == "empty")) {
        ...
        else if (averageSudsResult == "empty" && (averageVoCResult != "empty" && VoCResultsData.length > 0)) {
            document.getElementById("analysis-suds").className = "col col-12 margin-top-large hide-analysis";
            document.getElementById("analysis-VoC").className = "col col-12 col-lg-12 margin-top-large visible";
        ...
        dataVoC = {
            ...
            datasets: [{
                ...
                borderWidth: 3,
                borderColor: '#4257b2',
                data: convertToInt(VoCResultsData)
            }]
        };
...
function calculateSessionCount(array, type) {
    ...
    if (type == "mood") {
        ...
        for (var i = 0; i < array.length; i++) {
            if (array[i] != "empty") {
                if (i == 0) {
                    VoCResultsArray.push("Initial");
                }
                else {
                    VoCResultsArray.push(i.toString());
                }
            }
        }
        console.log("VoC: " + VoCResultsArray);
    }
}
