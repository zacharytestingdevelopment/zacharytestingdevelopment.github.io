...
function createAccount() {
    ...
    firebase.auth().createUserWithEmailAndPassword(emailValue, passwordValue).catch(function (error) {
        ...
        if (isAccountValid != false) {
            ...
            socket.emit('socketAction', { socketActionType: "createAccount", customerUsed: firebase.auth().currentUser.uid, pageUsed: currentPage, customerFirstName: firstNameValue, customerLastName: lastNameValue, customerEmail: emailValue });
        }
        else {
...
firebase.auth().onAuthStateChanged(user => {
    ...
    if (user) {
        ...
        if (pageActive != "signup" && pageActive != "signin" && pageActive != "dashboard") {
            socket.emit('socketAction', { socketActionType: "checkActiveCustomer", customerID: firebase.auth().currentUser.uid, pageUsed: pageActive });
        }
        ...
        if (pageActive == "dashboard") {
            desc.once('value', function (snapshot) {
                ...
                socket.emit('socketAction', { socketActionType: "checkActiveDashboard", customerID: firebase.auth().currentUser.uid })
...
form.addEventListener('submit', function (event) {
    ...
    //socket.emit('socketAction', { socketActionType: "addPaymentInformation", customerUsed: firebase.auth().currentUser.uid, pageUsed: currentPage });
...
function stripeTokenHandler(token) {
    ...
    socket.emit('socketAction', { socketActionType: "addPaymentInformation", tokenUsed: token, customerUsed: firebase.auth().currentUser.uid, planUsed: planHighlighted });
}

socket.on('userAction', function (data) {
    if (data.actionTaken == "planFinished") {
        if (data.userFinished == firebase.auth().currentUser.uid) {
            setInterval(function () { window.location = "dashboard.html"; }, 1000);
        }
    }
    else if (data.actionTaken == "dashboardTextUpdate") {
        if (data.currentUser == firebase.auth().currentUser.uid) {
            if (data.subscriptionStatus == "trialing") {
                //console.log("Text change: trialing");
                ...
                //console.log("Text change: active");
    ...
    else if (data.actionTaken == "canIBill") {

        if (data.subscriptionStatus == "cantbill") {
            if (firebase.auth().currentUser.uid == data.currentUser) {
                swal("Oops!", "You already have an active subscription!", "warning");
            }
        }
        else if (data.subscriptionStatus == "canbill") {
            if (firebase.auth().currentUser.uid == data.currentUser) {
                $('#paymentModal').modal({
                    backdrop: 'static',
                    keyboard: false  // to prevent closing with Esc button (if you want this too)
    ...
    else if (data.actionTaken == "subscriptionStatus") {
        if (data.currentUser == firebase.auth().currentUser.uid) {
            if (data.yourPage == "plans") {
                console.log("Your subscription on plans page: " + data.subscriptionStatus);
                if (data.subscriptionStatus == "trialing") {
            ...
            else if (data.yourPage == "emdr-analysis") {
                if (data.subscriptionStatus == "active" || data.subscriptionStatus == "trialing") {
                    console.log("You have an active subscription");
    ...
    else if (data.actionTaken == "subscriptionCanceled") {
        console.log("Canceled for: " + data.currentUser);
        if (data.currentUser == firebase.auth().currentUser.uid) {
    ...
    else if (data.actionTaken == "continueSubscription") {
        console.log("Continued for: " + data.currentUser);
        if (data.currentUser == firebase.auth().currentUser.uid) {
    ...
    else if (data.actionTaken == "accountDeleted") {
...
function checkIfCanBill(plan) {
    planHighlighted = plan;
    socket.emit('socketAction', { socketActionType: "checkTheBilling", billingCheck: true, customerID: firebase.auth().currentUser.uid });
}
...
function cancelSubscription(action) {
    ...
    if (action == "keepaccount") {
        socket.emit("socketAction", { socketActionType: 'cancelSubscription', cancelState: "keepaccount", currentUser: firebase.auth().currentUser.uid });
    }
    else if (action == "deleteaccount") {
        socket.emit("socketAction", { socketActionType: 'cancelSubscription', cancelState: "deleteAccount", currentUser: firebase.auth().currentUser.uid });
    }
    else if (action == "continuesubscription") {
        socket.emit("socketAction", { socketActionType: 'cancelSubscription', cancelState: "continueSubscription", currentUser: firebase.auth().currentUser.uid });
    }
    ...
    if (action == "keepaccount") {
        desc.once('value', function (snapshot) {
            console.log(snapshot.val().customerID);
            socket.emit('cancelSubscription', { customerID: snapshot.val().customerID, cancelState: 'keepaccount' });
        });
    }
    else if (action == "deleteaccount") {
        desc.once('value', function (snapshot) {
            console.log(snapshot.val().customerID);
            socket.emit('cancelSubscription', { customerID: snapshot.val().customerID, cancelState: 'deleteaccount' });
        });
    }
    else if (action == "continuesubscription") {
        desc.once('value', function (snapshot) {
            console.log(snapshot.val().customerID);
            socket.emit('cancelSubscription', { customerID: snapshot.val().customerID, cancelState: 'continuesubscription' });
        });
    }
    */
